...
mesh_name = "RectangleMesh"
mesh_file = RectangleMesh(Point(0, 0), Point(0.35, 0.02), 20, 20, "right")
...
if checkpoint == "FSI_fresh_checkpoints/CSM-1/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvFile.h5":
    sys.exit(0)
else:
    dv_file=HDF5File(mpi_comm_world(), "FSI_fresh_checkpoints/CSM-1/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvFile.h5", "w")
...
def initiate(t, T, args, mesh_file, mesh_name, \
    ...
    return dict(u_file=u_file, d_file=d_file, path=path)
...
def after_solve(t, path, dv_, n,coord,dis_x,dis_y, \
                counter,u_file,d_file, **semimp_namespace):
