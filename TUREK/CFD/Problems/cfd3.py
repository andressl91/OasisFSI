...
def initiate(args, dt, theta, mesh_name, vp_, **monolithic):
    ...
    for tmp_t in [u_file, p_file]:
        tmp_t.parameters["flush_output"] = True
        tmp_t.parameters["multi_file"] = 0
        tmp_t.parameters["rewrite_function_mesh"] = False
    v = vp_["n-1"].sub(0, deepcopy=True)
    p = vp_["n-1"].sub(1, deepcopy=True)
    #p_file.write(p)
    #u_file.write(v)
    u_file << v
    p_file << p

    return dict(path=path, u_file=u_file, p_file=p_file)
...
def after_solve(t, vp_, n, Drag_list,Lift_list, \
                u_file, p_file, **semimp_namespace):
