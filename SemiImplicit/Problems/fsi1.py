...
def create_bcs(DVP, D, V, args, dvp_, v_, n, k, w_f, Um, H, boundaries, inlet, **semimp_namespace):
    ...
    #Extrapolation bcs
    d_tilde = dvp_["n"].sub(0, deepcopy=True)
    d_n1 = dvp_["n-1"].sub(0, deepcopy=True)
    #w_bar = 1./k*(d_tilde("+") - d_n1("+"))
    w_bar = 1./k*(d_tilde - d_n1)

    wm_inlet  = DirichletBC(D, ((0.0, 0.0)), boundaries, 3)
    wm_outlet = DirichletBC(D, ((0.0, 0.0)), boundaries, 4)
    wm_wall   = DirichletBC(D, ((0.0, 0.0)), boundaries, 2)
    wm_circ   = DirichletBC(D, ((0.0, 0.0)), boundaries, 6)
    wm_bar    = DirichletBC(D,  w_bar, boundaries, 5)
    ...
    u_circ_t   = DirichletBC(V, ((0.0, 0.0)), boundaries, 6) #No slip on geometry in fluid
    u_bar_t    = DirichletBC(V, w_f, boundaries, 5) #No slip on geometry in fluid
    ...
    if args.bitype == "bc1":
        ...
        for i in [d_wall, d_inlet, d_outlet, d_circle, d_barwall, u_barwall]:
            bcs_solid.append(i)
