...
def create_bcs(DVP, V, args, dvp_, v_, n, k, Um, H, boundaries, inlet, **semimp_namespace):
    print "Create bcs"
    #Fluid velocity conditions
    #te = v_["n-1"]
    u_test  = DirichletBC(DVP.sub(1), dvp_["n-1"].sub(1, deepcopy=True), boundaries, 5)
    #u_test  = DirichletBC(DVP.sub(1), dvp_["n-1"].sub(1, deepcopy=True), boundaries, 3)
    ...
    u_inlet_t  = DirichletBC(DVP.sub(1).collapse(), inlet, boundaries, 3)
    u_wall_t   = DirichletBC(DVP.sub(1).collapse(), ((0.0, 0.0)), boundaries, 2)
    u_circ_t   = DirichletBC(DVP.sub(1).collapse(), ((0.0, 0.0)), boundaries, 6) #No slip on geometry in fluid
    u_barwall_t = DirichletBC(DVP.sub(1).collapse(), ((0.0, 0.0)), boundaries, 7) #No slip on geometry in fluid

    bcs_t = [u_wall_t, u_inlet_t, u_circ_t, u_barwall_t]
    ...
    if args.bitype == "bc1":
        ...
        for i in [d_wall, d_inlet, d_outlet, d_circle, d_barwall]:
            bcs.append(i)
        d_wall_e    = DirichletBC(DVP.sub(0).collapse(), ((0.0, 0.0)), boundaries, 2)
        d_inlet_e   = DirichletBC(DVP.sub(0).collapse(), ((0.0, 0.0)), boundaries, 3)
        d_outlet_e  = DirichletBC(DVP.sub(0).collapse(), ((0.0, 0.0)), boundaries, 4)
        d_circle_e  = DirichletBC(DVP.sub(0).collapse(), ((0.0, 0.0)), boundaries, 6)
        d_barwall_e = DirichletBC(DVP.sub(0).collapse(), ((0.0, 0.0)), boundaries, 7) #No slip on geometry in fluid
        for i in [d_wall_e, d_inlet_e, d_outlet_e, d_circle_e, d_barwall_e]:
            bcs_extra.append(i)
    ...
    return dict(bcs = bcs, bcs_t=bcs_t, bcs_extra=bcs_extra, inlet = inlet)
