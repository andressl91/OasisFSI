...
#RUN THIS *!!!
#python semi_implicit_linear.py -problem fsi1_fluid_only -fluidvari fluid_only -solver fluid_only -dt 0.01 -T 0.1
############
...
plot(mesh_file, interactive=True)
...
#"checkpoint": "./FSI_fresh_checkpoints/FSI_1-fluidonly/P-2/dt-0.05/dvpFile.h5"
vars().update(common)
...
def initiate(v_deg, d_deg, p_deg, dt, theta, vp_, args, mesh_name, refi, **semimp_namespace):
    ...
    if args.extravari == "alfa":
        path = "FSI_fresh_results/FSI_1-fluidonly/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
    if args.extravari == "biharmonic" or args.extravari == "laplace" or args.extravari == "elastic" or args.extravari == "biharmonic2":
        path = "FSI_fresh_results/FSI_1-fluidonly/%(exva)s_%(bitype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
...
def create_bcs(dw_, d_, DW, VP, args, k, Um, H, boundaries, inlet, **semimp_namespace):
    ...
    u_bar_t    = DirichletBC(VP.sub(0), ((0.0, 0.0)), boundaries, 5) #No slip on geometry in fluid

    bcs_tent = [u_wall_t, u_inlet_t, u_circ_t]
    ...
    u_bar    = DirichletBC(VP.sub(0), ((0.0, 0.0)), boundaries, 5) #No slip on geometry in fluid

    p_outlet  = DirichletBC(VP.sub(1), (0.0), boundaries, 4)

    #Assemble boundary conditions
    bcs_corr = [u_inlet, u_wall, u_circ, u_bar, \
                p_outlet]
