...
def domain_update(DW, w_, d_, dw_,  k, dt, mesh_file, **semimp_namespace):
	...
	d_n =  dv_["tilde"].sub(0, deepcopy=True)
	d_n1 = dv_["n-1"].sub(0, deepcopy=True)
	w_n1 = dv_["n-1"].sub(1, deepcopy=True)
	w_n2 = dv_["n-2"].sub(1, deepcopy=True)

	d_n.vector().zero()
	d_n.vector().axpy(1, d_n1.vector() \
	            + dt*(3./2*w_n1.vector() - 1./2*w_n2.vector()) )
	d = DW.sub(0).dofmap().collapse(mesh_file)[1].values()
	dv_["tilde"].vector()[d] = d_n.vector()
