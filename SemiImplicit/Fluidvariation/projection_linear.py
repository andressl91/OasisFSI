...
def Fluid_tentative_variation(v_, p_, d_, vp_, w, v_f, v_tilde_n1, \
psi, beta, gamma, dx_f, mu_f, rho_f, k, dt, **semimp_namespace):
	...
	F_tentative = rho_f/k*J_(d_tent)*inner(v_f - v_["n-1"], beta)*dx_f

	F_tentative = rho_f*inner(J_(d_["tent"])*grad(v_f)*inv(F_(d_["tent"])) \
	             * (v_["tilde-1"] - w_["n"]), beta)*dx_f

	F_tentative += J_(d_["tent"])*inner(sigma_f_u(v_f, d_["tent"], mu_f), grad(beta))*dx_f
