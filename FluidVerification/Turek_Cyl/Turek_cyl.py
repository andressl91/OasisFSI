...
def fluid(mesh_file, T, dt, solver, steady, fig, v_deg, p_deg):
    ...
    Nos.mark(boundaries, 1) #NOSLIP BOUNDARY
    ...
    #STEADY FLOW
    inlet = Expression(("4*Um*x[1]*(H - x[1]) / (H*H)"\
    ,"0"), Um = Um, H = H)

    u_inlet = DirichletBC(VQ.sub(0), inlet, boundaries, 2)
    noslip = DirichletBC(VQ.sub(0), ((0, 0)), boundaries, 1)

    p_out = DirichletBC(VQ.sub(1), 0, boundaries, 3)

    bcs = [u_inlet, noslip, p_out]
