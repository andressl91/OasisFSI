...
"For details: http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.550.1689&rep=rep1&type=pdf",\
 formatter_class=RawTextHelpFormatter, \
  ...
  "Example --> python ALE_FSI_adam.py -v_deg 2 -p_deg 1 -d_deg 2 -r -dt 0.5 -T 10 -step 10 -FSI_number 1  (Refines mesh one time, -rr for two etc.) \n"
  "############################################################################")
...
def P1(d,d0,d1):
	return (lamda_s/8)*((tr((3*grad(d0)-grad(d1))*(grad(d)-grad(d0)) - I ))*I + grad(d)+grad(d0) + 0.25*(grad(d) - grad(d0)) * (3*grad(d0).T - grad(d1).T))
...
# Structure var form
F_structure = (rho_s/k)*inner(u-u0,phi)*dx_s + inner(P1(d,d1,d0),grad(phi))*dx_s
