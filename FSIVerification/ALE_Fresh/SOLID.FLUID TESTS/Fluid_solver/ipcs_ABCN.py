...
def setup(u_, p_, n, u_sol, p_sol, v, q, dx, ds, mu_f, rho_f, nu, k, **semimp_namespace):

    U_cn = 0.5*(u_["n"] + u_["n-1"])
    U_ab = 3./2*u_["n-1"] -1/2.*u_["n-2"]
    ...
    + inner(sigma_f(U_cn, p_["n-1"], nu), sym(grad(v)) )*dx \
    - beta*nu*inner(dot(grad(U_cn).T, n), v)*ds + inner(p_["n-1"]*n ,v)*ds

    # Velocity update
    Fp_corr = dot(grad(p_["n"]), grad(q))*dx - dot(grad(p_["n-1"]), grad(q))*dx  \
    + 1./k*div(u_sol)*q*dx

    Fu_upt = 1./k*inner(u_["n"] - u_["n-1"], v)*dx \
    + dot(grad(p_sol - p_["n-1"]), v)*dx
