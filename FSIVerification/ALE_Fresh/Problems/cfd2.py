...
if checkpoint == "FSI_fresh_checkpoints/CFD-2/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5":
    sys.exit(0)
else:
    dvp_file=HDF5File(mpi_comm_world(), "FSI_fresh_checkpoints/CFD-2/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5", "w")
...
def initiate(P, v_deg, d_deg, p_deg, dt, theta, dvp_, args, Det_list, refi, mesh_file, mesh_name, **semimp_namespace):
    ...
    if args.extravari == "alfa":
        path = "FSI_fresh_results/CFD-2/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
    if args.extravari == "biharmonic" or args.extravari == "laplace" or args.extravari == "elastic":
        path = "FSI_fresh_results/CFD-2/%(exva)s_%(bitype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
