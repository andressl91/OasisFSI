...
def initiate(v_deg, d_deg, p_deg, dt, theta, dvp_, args, mesh_name, refi, **semimp_namespace):
 ...
 if args.extravari == "alfa":
     path = "FSI_fresh_results/CFD-3/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
 if args.extravari == "biharmonic" or args.extravari == "laplace" or args.extravari == "elastic":
     path = "FSI_fresh_results/CFD-3/%(exva)s_%(bitype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
