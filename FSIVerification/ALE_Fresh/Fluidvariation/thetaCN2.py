...
def fluid_setup(v_, p_, d_, n, psi, gamma, dx_f, ds, mu_f, rho_f, k, dt, v_deg, theta, **semimp_namespace):
    ...
    F_fluid_nonlinear = rho_f/k*inner(theta*J_(d_["n"])*v_["n"] \
                            - theta*(J_(d_["n"])*v_["n-1"]) \
                            + (1 - theta)*J_(d_["n-1"])*v_["n"], psi)*dx_f

    F_fluid_linear = rho_f/k*Constant(1 - theta)*inner(J_(d_["n"])*v_["n-1"], psi)*dx_f
